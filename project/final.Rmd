...
[^1]:Basically, `for` loops are functions themselves. R prints out the result of a command automatically, but functions are not inherently a command, and will therefore not print anything unless explicitly run. The solution is to have `print(command())` to get output for your `for` loop. You will never again spend hours trying to find out why a `for` loop doesn't print anything because you're no longer an R newbie.
...
for (i in seq_along(samples_of_countries))
{
	...
	list.append(countries_to_test, current_samples)
}
