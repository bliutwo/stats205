...
[^1]:Basically, `for` loops are functions themselves. Since R prints out the result of a command automatically, but functions are not inherently a command (unless explicitly run), you need to have `print(command())` in order to get output. You will never again spend hours trying to find out why a `for` loop doesn't print anything because you're no longer an R newbie. 
