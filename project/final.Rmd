...
unique(filtered_suicide_df$continent)
```

Notably, there are countries that are not on any of the listed continents. Let's see which ones those are:
...
head(europe_suicide)
americas_suicide = filtered_suicide_df[filtered_suicide_df$continent == 'Europe',]
```
