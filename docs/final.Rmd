...
{
	cat(i)
	cat(toString(list_of_continents[i]))
	cat(list_of_continents[i])
	cont_string = toString(list_of_continents[i])
	cat(cont_string)
	cat("\n")

	countries_per_continent[[i]] <- filtered_suicide_df[filtered_suicide_df$continent == cont_string,]
	print(head(countries_per_continent[[i]]))

	continent <- filtered_suicide_df[filtered_suicide_df$continent == cont_string,]
	head(continent)

	# why does THIS not work if that works?
	continent_eur <- filtered_suicide_df[filtered_suicide_df$continent == 'Europe',]
	head(continent_eur)

	# LITERALLY COPY PASTING IT, and it still doesn't work
	stringerino <- "Europe"
	asdfg <- filtered_suicide_df[filtered_suicide_df$continent == stringerino,]
	head(asdfg)
}
...
countries_per_continent[0] <- filtered_suicide_df[filtered_suicide_df$continent == cont_string,]
head(countries_per_continent[0])

length(countries_per_continent)
# head(countries_per_continent)
for (i in seq_along(countries_per_continent))
{
	print(head(countries_per_continent[[i]]))
	print(head(countries_per_continent[i]))
	typeof(countries_per_continent[[i]])
	typeof(countries_per_continent[i])
	cat("\n")
}
```

[This text links to very important information about why for loop doesn't print anything.](https://stackoverflow.com/questions/14987743/head-function-does-not-work-within-for-loop) 
