...
```{r}
wilcox.test(x = a, y = b, alternative = "greater")
```

The $p$-value is $0.000386$, which is significant at the $\alpha = 0.05$ level. There is `fill in this blank` evidence that Type A subjects have different Peak Levels of Human Plasma Growth Hormone after Arginine Hydrochloride Infusion as Type B subjects.